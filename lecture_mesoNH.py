                        ...
                        for param in params:
                            if param not in data_mnh[today_str][model]:
                                ...
                                if param == "tmp_10m":
                                    ...
                                    D = (100000/P)**(2/7)  
                                    data_mnh[today_str][model][param] = (f[groupMEAN].variables['MEAN_TH'][:, 5]/ D)-273.15
                                if param == "hum_rel":                                    
                                    data_mnh[today_str][model][param] = f[groupMEAN].variables['MEAN_REHU'][:,5]

                                if param == "vent_ff10m":
                                    data_mnh[today_str][model][param]= f[groupMEAN].variables['MEANWIND'][:,5]
                                if param == "flx_mvt":
                                    data_mnh[today_str][model][param] = f[groupSURF].variables['Ustar'][:]
                                if param == "flx_chaleur_sens":
                                    data_mnh[today_str][model][param] = f[groupSURF].variables['Q0'][:]*f.variables['RHODREF'][2]*1004
                                if param == "flx_chaleur_lat":
                                    data_mnh[today_str][model][param] = f[groupSURF].variables['E0'][:]*f.variables['RHODREF'][2]*2400000 #TODO facteur a corriger
                                if param == "LWU":
