...
def surfex (start_day,end_day,models,params):
    ...
    for i in range(nb_jour+1):
        ...
        if today_str not in data_surfex :
            ...
            for model in models:
                try :
                    f = nc.Dataset('/cnrm/ktrm/stagiaire/mosai_2021/DEV/SURFEX/'+today_str+'/SURFEX_'+today_str+'_'+model+'.nc')
                    ...
                    if model not in data_surfex[today_str]:
                        ...
                        for param in params:
                            if param not in data_surfex[today_str][model]:
                                ...
                                if param == "tmp_2m":
                                    data_surfex[today_str][model][param] = f['T2M'][:,0,0][:24]-273.15
                                if param == "tmp_10m":
                                    data_surfex[today_str][model][param] = []
                                if param == "vent_ff10m":
                                    data_surfex[today_str][model][param] = []
                                if param == "hum_rel":
                                    data_surfex[today_str][model][param] = f['HU2M'][:,0,0][:24]*10000		#TODO conversion HU 
#                                if param == "flx_mvt":
#                                    data_surfex[today_str][model][param] = []
