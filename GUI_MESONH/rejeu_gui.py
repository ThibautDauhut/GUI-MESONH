...
class get_mesonh_gui():
    def fill_user_params(self,userparam,options):
    ...
    def start_callbacks(self,app):
      ...
      for nam in self.Options.keys():
          for divName in self.Options[nam]['divName']:
              for component in self.Options[nam]['divsvalues'][divName].children:
      ...
      def simu(n_clicks):
          if n_clicks is not None and n_clicks == 1:
              self.user_params["id"] = shortuuid.uuid()[:4]
              content = [html.H5([html.Span('Simulation '), 
                         html.Span(self.user_params["id"], style={"font-weight": "bold"}),
      ...
      def loader_func(n_clicks, start_date, end_date, *arg):
          if start_date is not None: #First call of the app
              ...
              if n_clicks is not None and n_clicks == 1:
                  ...
                  user_params_id = self.user_params
                  user_params = self.fill_user_params(user_params_id,self.Options)
